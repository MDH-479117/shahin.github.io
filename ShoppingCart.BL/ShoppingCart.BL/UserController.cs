...
namespace ShoppingCart.BL
{
    public class UserController
    {
        ...
        {
            return (SqlHelper.ExecuteDataset(ConnectionString.GetConnectionString(), CommandType.StoredProcedure, "spGetALLUsers"));
        }
        public static DataSet GetUserBYUserID(int UserID)
        {
            SqlParameter p = new SqlParameter("@UserID", UserID);
            return (SqlHelper.ExecuteDataset(ConnectionString.GetConnectionString(), CommandType.StoredProcedure, "spGetUserBYUserID",p));
        }
        public static void UpdateUserDetails(string p_username, string p_fname,string p_lname, DateTime p_dob, string p_no, string p_gender, string p_email,string p_address, int p_country, int p_state,int p_city, string p_zcode,int AddrID,int UID)
        {
